...
class PatchTransformer(nn.Module):
    ...
    def generate_light_spot(self, patch_size):
        # Generate a random spot size (up to 1/5 of the patch size)
        spot_size = np.random.randint(1, patch_size // 4)  # 减小 spot_size 的范围

        # Generate a grid of coordinates
        y, x = np.ogrid[-spot_size:spot_size + 1, -spot_size:spot_size + 1]
        # Create a circular mask
        distance_from_center = np.sqrt(x**2 + y**2)
        max_distance = np.sqrt(2) * spot_size  # 计算最大距离
        circle = np.clip(1 - (distance_from_center / max_distance), 0, 1)  # 确保中心是白色，边缘逐渐减少
        # 将单通道的高斯分布扩展到三个通道
        light_spot = np.stack((circle, circle, circle), axis=-1)

        return torch.tensor(light_spot, dtype=torch.float32, device=self.dev)



    def forward(
        self, adv_patch, lab_batch, model_in_sz, use_mul_add_gau=True, do_transforms=True, do_rotate=True, rand_loc=True,epoch=None
    ):
        ...
        # #Add light spots to the patches
        # for i in range(batch_size[0]):
        #     # 随机选择一个检测框来添加光斑
        #     if lab_batch[i].size(0) > 0:  # 确保有检测框
        #         j = np.random.randint(0, lab_batch[i].size(0))
        #         light_spot = self.generate_light_spot(p_h)
        #         # 调整 light_spot 的形状为 [3, pheight, pwidth]
        #         light_spot = light_spot.permute(2, 0, 1)
        #         # Randomly place the light spot within the patch
        #         x_start = np.random.randint(0, p_h - light_spot.size(-2) + 1)
        #         y_start = np.random.randint(0, p_w - light_spot.size(-1) + 1)
        #         adv_batch[i, j, :, x_start:x_start + light_spot.size(-2), y_start:y_start + light_spot.size(-1)] += light_spot
        #         break

             # 打印并保存第一次应用变换后得到的补丁
        if self.training and epoch  == 1:
            first_patch = adv_batch[0, 0].cpu().detach().numpy().transpose(1, 2, 0)
            first_patch = (first_patch * 255).astype(np.uint8)
            img = Image.fromarray(first_patch)
            img.save(osp.join("runs/train_adversarial", "first_spotlight_patch_1.png"))
            #img.show()
        ...
        # #打印并保存第一个叠加的补丁
        # if epoch == 1 and self.training:
        #     first_patch = adv_batch_t[0, 0].cpu().detach().numpy().transpose(1, 2, 0)
        #     first_patch = (first_patch * 255).astype(np.uint8)
        #     img = Image.fromarray(first_patch)
        #     img.save(osp.join("runs/train_adversarial", "first_nested_patch_1.png"))
        ...
        if epoch == 1 and self.training:
            ...
            first_patch = (first_patch * 255).astype(np.uint8)
            img = Image.fromarray(first_patch)
            img.save(osp.join("runs/train_adversarial", "first_transformed_patch_3.png"))
