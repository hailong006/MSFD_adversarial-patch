...
"""
"""
216-change methods bicubic 
89-change gaussian and color doud 
234-jpg-png clarity RGBA
285-add_img
315-random_spot
...
from texture import dynamic_interference, anti_motion_blur 
...
from adv_patch_gen.utils.loss_freq_mul_dist import MaxProbExtractor, NPSLoss, SaliencyLoss, TotalVariationLoss, FrequencyLoss, DistributionLoss, MultiScaleEntropyConsistencyLoss
from adv_patch_gen.utils.patch_add_lightspot_pure import PatchApplier, PatchTransformer
...
class PatchTrainer:
    ...
    def __init__(self, cfg: edict):
        ...
        self.freq_loss = FrequencyLoss(cutoff_ratio=0.3).to(self.dev)
        self.dist_loss = DistributionLoss(
            method='activation_entropy', 
            lambda_dist=getattr(cfg, 'dist_mult', 0.01)  # Default weight
        ).to(self.dev)
        self.msec_loss = MultiScaleEntropyConsistencyLoss(
            scale_factors=[0.9, 0.7, 0.5],
            alpha=1.0,
            beta=0.5
        ).to(self.dev)
        ...
        # self.dist_loss = DistributionLoss(
        #     method='activation_entropy', 
        #     lambda_dist=getattr(cfg, 'dist_mult', 0.1)  # Default weight 0.1
        # ).to(self.dev)

        # self.dist_loss = DistributionLoss(
        #     method='spatial_distribution', 
        #     lambda_dist=getattr(cfg, 'dist_mult', 0.1)  # 默认权重0.1
        # ).to(self.dev)
        ...
        if cfg.augment_image:
            transforms = T.Compose(
                [
                    ...
                    T.GaussianBlur(kernel_size=(3, 3), sigma=(0.1, 0.5)),  # Reduce sigma
                    T.ColorJitter(brightness=0.1, hue=0.02, contrast=0.05),  # Reduce intensity
                    T.RandomAdjustSharpness(sharpness_factor=1.5),  # Reduce sharpness_factor
                ]
            )
    ...
    def generate_patch(self, patch_type: str, pil_img_mode: str = "RGB") -> torch.Tensor:
        ...
        """
        p_c = 1 if pil_img_mode in {"L"} else 3  # Determine channel count: 1 for grayscale, 3 for RGB
        p_w, p_h = self.cfg.patch_size  # Get patch width and height
        if patch_type == "gray":
            adv_patch_cpu = torch.full((p_c, p_h, p_w), 0.5)  # Create tensor filled with 0.5
        elif patch_type == "random":
            adv_patch_cpu = torch.rand((p_c, p_h, p_w))   # Create tensor with random values
    ...
    def train(self) -> None:
        ...
        if self.cfg.debug_mode:
            for img_dir in ["train_patch_applied_imgs", "val_clean_imgs", "val_patch_applied_imgs"]:
                ...
                # Ensure subdirectories are also created
                os.makedirs(osp.join(self.cfg.log_dir, img_dir, "images"), exist_ok=True)
        ...
        # Save the initially generated patch
        img = T.ToPILImage(self.cfg.patch_img_mode)(adv_patch_cpu)
        img.save(osp.join(self.cfg.log_dir, "initial_patch.png"))
        #img.show()
            ...
            for i_batch, (img_batch, lab_batch) in tqdm(
                ...
                with autograd.set_detect_anomaly(mode=True if self.cfg.debug_mode else False):
                    ...
                    adv_batch_t = self.patch_transformer(
                        ...
                        epoch=epoch  # Pass epoch information
                    )
                    p_img_batch = self.patch_applier(img_batch, adv_batch_t)
                    p_img_batch = F.interpolate(p_img_batch, (self.cfg.model_in_sz[0], self.cfg.model_in_sz[1]),mode="bicubic")#double linear interpolation
                    ...
                    if self.cfg.debug_mode:
                        ...
                        img.save(osp.join(self.cfg.log_dir, "train_patch_applied_imgs", f"b_{i_batch}.png")) # high resolution

                    with autocast(device_type='cuda') if self.cfg.use_amp else nullcontext():
                        ...
                        freq = self.freq_loss(adv_patch) if self.cfg.freq_mult != 0 else zero_tensor 
                        
                   

                    det_loss = torch.mean(max_prob)
                    dist = self.dist_loss(adv_patch) if self.cfg.dist_mult != 0 else zero_tensor
                    dist_loss = dist * self.cfg.dist_mult if self.cfg.dist_mult != 0 else zero_tensor

                    msec = self.msec_loss(adv_patch) if getattr(self.cfg, 'msec_mult', 0) != 0 else zero_tensor
                    msec_loss = msec * getattr(self.cfg, 'msec_mult', 0.3)  




                    sal_weight = self.cfg.sal_mult * (0.5 ** (epoch // 20))
                    sal_loss = sal * sal_weight
                    nps_loss = nps * self.cfg.nps_mult 
                    tv_loss = torch.max(tv * self.cfg.tv_mult, min_tv_loss)
                    freq_loss = freq * self.cfg.freq_mult  
                    final_det_loss = self.cfg.loss_target(det_loss, max_prob)
                    if hasattr(final_det_loss, 'mean'):
                        final_det_loss = torch.mean(final_det_loss)

    
                    # det_loss = self.cfg.loss_target(det_loss, max_prob)
                    #tf_loss = tv_loss + freq_loss


                    #loss = final_det_loss + nps_loss + sal_loss + tv_loss + dist_loss  # Combine total loss
                    loss = final_det_loss + nps_loss + sal_loss + tv_loss + msec_loss  # Combine total loss
                    if hasattr(loss, 'dim') and loss.dim() > 0:
                        loss = torch.mean(loss)

    
                    #loss = det_loss + sal_loss + nps_loss  + tv_loss  # Combine total loss
                    ep_loss += loss
                    ...
                    if i_batch % self.cfg.tensorboard_batch_log_interval == 0:
                        ...
                        self.writer.add_scalar("loss/det_loss", final_det_loss.detach().cpu().numpy(), iteration)
                        self.writer.add_scalar("loss/sal_loss", sal_loss.detach().cpu().numpy(), iteration)
                        self.writer.add_scalar("loss/nps_loss", nps_loss.detach().cpu().numpy(), iteration)
                        self.writer.add_scalar("loss/tv_loss", tv_loss.detach().cpu().numpy(), iteration)
                        self.writer.add_scalar("loss/freq_loss", freq_loss.detach().cpu().numpy(), iteration)  # New log
                        self.writer.add_scalar("loss/dist_loss", dist_loss.detach().cpu().numpy(), iteration)  # New log
                        self.writer.add_scalar("loss/msec_loss", msec_loss.detach().cpu().numpy(), iteration)  # New log
                        #self.writer.add_scalar("loss/tf_loss", tf_loss.detach().cpu().numpy(), iteration) 
                        self.writer.add_scalar("misc/epoch", epoch, iteration)
                        self.writer.add_scalar("misc/learning_rate", optimizer.param_groups[0]["lr"], iteration)
                        self.writer.add_image("patch", adv_patch_cpu, iteration)
                    if i_batch + 1 < len(self.train_loader):
                        del adv_batch_t, output, max_prob, det_loss, p_img_batch, sal_loss, nps_loss, dist_loss,tv_loss, loss
    ...
    def val(self, epoch: int, patchfile: str, conf_thresh: float = 0.5, nms_thresh: float = 0.5) -> None:
        ...
        adv_patch = adv_patch_cpu.to(self.dev)
         ...
         # Ensure validation image directory exists
        if not osp.exists(self.cfg.val_image_dir):
            raise FileNotFoundError(f"Validation image directory does not exist: {self.cfg.val_image_dir}")
        ...
        #print(f"Validation image paths: {img_paths}")  # Debug print


        train_t_size_frac = self.patch_transformer.t_size_frac
        self.patch_transformer.t_size_frac = [0.25, 0.4]  # use a frac of 0.3 for validation
        ...
        '''Save log'''
        log_path = osp.join(self.cfg.log_dir, "tran_log.txt")
        with open(log_path, "a") as f:
            f.write(f"Epoch {epoch}:\n")
            f.write(f"Validation metrics for images with patches:\n")
            f.write(f"\tASR@thres={conf_thresh}: asr_s={asr_s:.3f}, asr_m={asr_m:.3f}, asr_l={asr_l:.3f}, asr_a={asr_a:.3f}\n\n")
...
def main():
    ...
    config_path = "adv_patch_gen/configs/base.json"
    cfg = load_config_object(config_path)
    trainer = PatchTrainer(cfg)
    trainer.train()
