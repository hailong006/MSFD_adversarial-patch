...
def run(
    ...
    if len(stats) and stats[0].any():
        ...
        if weights:
            ...
            # Extract model name from weights path safely
            weights_path = str(weights[0] if isinstance(weights, list) else weights)
            weights_parts = weights_path.split("/")
            if len(weights_parts) >= 3:
                mname = "_".join(weights_parts[2].split("_")[:2])
            else:
                # For simple paths like "detect_best.pt", use the filename without extension
                mname = Path(weights_path).stem
...
def parse_opt():
    ...
    parser.add_argument("--data", type=str, default=ROOT / "data/patch_test.yaml", help="dataset.yaml path")
    parser.add_argument("--weights", nargs="+", type=str, default=ROOT / "detect_best.pt", help="model path(s)")
